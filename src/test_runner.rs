...
impl TestRunner {
    ...
    pub async fn run_tests(&self, tests: Vec<Box<dyn LoadTest>>) {
        for test in tests {
            ...
            for i in 0..10 {
                ...
                let num_writes = (10 as usize).pow(i);
                let result = test.run(num_writes, 1000..10_000).await;
                ...
                if run_duration_ns / 1_000_000_000.0 > 1.0 {
                    break;
                }
            }
    ...
    fn print_results(result: TestResult, num_writes: usize, run_duration_ns: f64) {
        ...
        // println!("Write Ops Time ({} writes, {:.2} ms, {:.2} writes/s):", result.num_writes, run_duration_ms, writes_per_s);
